...
image_name = filedialog.askopenfilename(initialdir = '../data')
#image_name = input("Please input the name of your image file:")
...
comboxlist["values"]=("摄像头","选择视频文件")
comboxlist.current(0)  #选择第一个 
video_name=""
def go(*args):
    choose=comboxlist.get()
    global video_name
    if choose == 0:
        video_name=""
    else:
        video_name=filedialog.askopenfilename(initialdir = '../data')
comboxlist.bind("<<ComboboxSelected>>",go)
comboxlist.pack()
 
win.mainloop() #进入消息循环

#video_name = input("Please input the name of your video file, or input an Enter if you want to use PC camera:")
if video_name == "":
...
while True:
    ...
    if key == ord('r'):
        if writer is None:
            ...
            writer = cv2.VideoWriter("../out3.avi", cv2.VideoWriter_fourcc('X', 'V', 'I', 'D'), 25, (cameraImg.shape[1], cameraImg.shape[0]))
